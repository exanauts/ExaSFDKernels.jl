...
@testset "dicfs" begin
    function dicfs_test(n::Int, alpha::Float64,
        ...
        ExaSGDKernels.dicfs(n, alpha, A, L, wa1, wa2)
        if bx == 1
            @inbounds for j=1:n
                d_out[j,tx] = L[j,tx]
            end
...
@testset "dtrpcg" begin
    function dtrpcg_test(n::Int, delta::Float64, tol::Float64,
        ...
        ExaSGDKernels.dicf(n,L)
        info, iters = ExaSGDKernels.dtrpcg(n,A,g,delta,L,tol,stol,n,w,p,q,r,t,z)
        if ty == 1
            d_out[tx] = w[tx]
        end
...
@testset "daxpy" begin
    function daxpy_test(n::Int, da, d_in::CuDeviceArray{Float64},
        ...
        ExaSGDKernels.daxpy(n, da, x, 1, y, 1)
        if ty == 1
            d_out[tx] = y[tx]
        end
        CUDA.sync_threads()
...
@testset "dtrqsol" begin
    function dtrqsol_test(n::Int, d_x::CuDeviceArray{Float64},
        ...
        sigma = ExaSGDKernels.dtrqsol(n, x, p, delta)
        d_out[tx,ty] = sigma
        CUDA.sync_threads()
    end
...
@testset "driver_kernel" begin
    ...
    function driver_kernel(n::Int, max_feval::Int, max_minor::Int,
        ...
        while search
            ...
            if task == 0
                gnorm0 = ExaSGDKernels.dnrm2(n, g, 1)
                delta = gnorm0
            end
            ...
            if search
                delta, task = ExaSGDKernels.dtron(n, x, xl, xu, f, g, A, frtol, fatol, fmin, cgtol,
                                            cg_itermax, delta, task, B, L, xc, s, indfree, gfree,
            ...
            if task == 3
                gnorm_inf = ExaSGDKernels.dgpnorm(n, x, xl, xu, g)
                if gnorm_inf <= gtol
                    task = 4
                end
    ...
    for i=1:itermax
        ...
        tron = ExaSGDKernels.createProblem(n, xl, xu, nele_hess, eval_f_cb, eval_g_cb, eval_h_cb; :matrix_type=>:Dense, :max_minor=>max_minor)
        copyto!(tron.x, x)
        status = ExaSGDKernels.solveProblem(tron)
